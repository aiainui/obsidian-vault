---
title: "Claude Code源码逆向工程与系统性分析！Harness Engineering:基于Claude Code的完全指南"
source: "https://mp.weixin.qq.com/s/Q5oK_VtunXlouachrj1agQ"
author:
published:
created: 2026-04-06
description: "本教程基于ClaudeCode源码（~512,664行TypeScript）的逆向工程与系统性分析，以教科书的体例深入剖析AIAgentHarness的每一个设计决策、工程取舍与实现细节。全文遵循学术写作规范，在代码分析之上构建理论框架，将零散的实现细节提炼为可复用的设计原则"
tags:
  - "clippings"
---
*2026年4月5日 21:19*
...
```
Harness EngineeringContext Engineering静态上下文CLAUDE.mdAGENTS.md设计文档动态上下文日志与指标Git 状态CI/CD 状态上下文压缩四级管道按需加载记忆系统Architectural Constraints权限模型5 种模式7 级规则层级AI 分类器工具约束Schema 验证并发安全标记延迟加载安全边界沙盒隔离硬编码拒绝纵深防御Entropy Management定期清理死代码检测文档一致性约束验证依赖审计模式强制性能监控覆盖率守卫回归检测
```

Three Pillars Time Allocation
...
```
进入循环压缩完成有 tool_use 块无 tool_use 块413 错误collapse 失败max_output_tokens升级后仍截断FallbackTriggeredErrorcontinue site 1continue site 2continue site 3continue site 4continue site 6continue site 7\n（正常下一轮）正常完成blocking error\ncontinue site 5hook 阻止继续恢复失败重试 3 次后耗尽CompactionAPICallToolExecutionStopHooksCollapseRetryReactiveCompactEscalateTokensMultiTurnRetryFallbackModel
```
...
```
压缩管道（每轮迭代执行）释放少量 token边界消息延迟如果仍超阈值如果低于阈值Level 1: Snip\n历史截断\n成本: 极低 | 延迟: ~0msLevel 2: Microcompact\n老化工具结果缩减\n成本: 低 | 延迟: ~1msLevel 3: Context-Collapse\n读时投射（不修改数组）\n成本: 中 | 延迟: ~5msLevel 4: Autocompact\nLLM 全对话摘要\n成本: 高 | 延迟: ~2s跳过 Autocompact\n保留粒度上下文
```
...
```
PostToolUse HookTool.call()SandboxPermission EnginePreToolUse HookValidatorModelPostToolUse HookTool.call()SandboxPermission EnginePreToolUse HookValidatorModelalt[验证失败]alt[验证失败]alt[Hook 阻止][Hook 修改输入][Hook 批准]alt[权限拒绝]tool_use blockZod Schema 验证格式错误消息tool.validateInput()业务逻辑错误输入 JSON条件匹配 (if 字段)blocking errorupdatedInputallow (但不绕过 deny 规则)deny规则 → ask规则 → 模式检查拒绝消息 + 建议命令包装 (仅 BashTool)wrapWithSandbox()沙盒化命令实际执行执行结果审计日志 / 输出修改最终 tool_result
```
...
```
第 6 层: Hardcoded Denials（不可覆盖约束）settings.json 始终不可写，无法通过配置禁用第 5 层: Sandbox（系统级约束）操作系统级文件/网络隔离第 4 层: YOLO Classifier（AI 约束）独立 AI 模型审查操作安全性第 3 层: Hooks（可编程约束）PreToolUse 脚本检查操作合法性第 2 层: Permission Rules（声明性约束）settings.json 中的 allow/deny/ask 规则第 1 层: CLAUDE.md（指导性约束）告诉 Agent '不要修改 migrations/ 目录'
```
...
```
Agent HookPrompt HookCommand HookHTTP Hook合规检查代码质量评分审计日志Slack 通知测试验证安全审查Lint 运行规则检查低成本高成本低智能高智能Hook 类型选择矩阵
```
...
> 
> 1. 1\. Agent 应该知道什么？（CLAUDE.md、记忆系统）
> 2. 2\. 什么时候告诉它？（按需加载、预取）
> 3. 3\. 信息太多了怎么办？（四级压缩管道）
> 
...
> 
> **缓存经济学** ：系统提示和工具定义作为缓存前缀（~30-46K tokens）。Anthropic API 的 prompt cache 对前缀匹配的部分不收取输入费用。按 0.0001-0.00014。一个典型会话有 20-50 次 API 调用，总节省约 $0.002-0.007/会话。规模化后（百万级日活），这是显著的成本优化。
...
```
Claude CodeStdio\n子进程 stdin/stdoutSSE\nServer-Sent EventsStreamable HTTP\nHTTP 流WebSocket\nTLS/代理支持InProcess\n内存 TS 模块SdkControl\nSDK daemon本地 MCP 服务器\n(filesystem, git)远程 MCP 服务器\n(Slack, Linear)Chrome/Computer Use\n避免 325MB 子进程SDK Daemon\n控制面
```
...
```
子 Agent 工具集子 AgentMCP 服务器AgentTool父 Agent子 Agent 工具集子 AgentMCP 服务器AgentTool父 Agentloop[每 500ms 轮询 (最多30s)]alt[MCP 未就绪]messages = []独立 Token 预算独立压缩管道loop[Agent 执行]Agent(type:"Explore", prompt:"...")加载 Agent 定义检查 MCP 服务器就绪连接状态?创建隔离上下文组装工具集 (定义指定或继承)独立 queryLoop()工具调用工具结果返回摘要摘要 (不含子 Agent 完整历史)
```
...
1. 1\. 全局状态（每个查询有自己的上下文）
2. 2\. 参数爆炸（不需要逐个传递）
3. 3\. 紧耦合（工具只使用自己需要的字段）

---

## 附录 D：13 条防护规则参考模型

claude-code-harness <sup>[1]</sup> 项目定义了 13 条声明性防护规则，作为 PreToolUse Hook 实现：
...
> 
> 1. 1\. **简单的 Hook 系统** ：在工具执行前后运行 shell 脚本
> 2. 2\. **Token 计数器** ：追踪每次 API 调用的 token 消耗和累计成本
> 3. 3\. **会话保存/恢复** ：将 messages 序列化到 JSON 文件，支持 `--resume`
> 4. 4\. **子 Agent** ：添加一个 `agent` 工具，分叉一个新的 agent\_loop() 带独立消息
...
1. 1\. **编译时特性门控** （bun:bundle dead code elimination）—— 竞品无此机制
2. 2\. **纵深防御六层安全模型** —— 最深的安全层次
3. 3\. **YOLO 两阶段分类器** —— 唯一使用独立 AI 审查权限的 Agent
4. 4\. **可逆工具设计** （Edit 用字符串替换而非覆盖）—— 降低破坏风险
5. 5\. **Prompt Cache 稳定性排序** —— 工具排序优化缓存命中率

**Cursor 的独特创新** ：

1. 1\. **代码库索引** （全局语义理解）—— Claude Code 依赖 Grep/Glob
2. 2\. **8 并行 Agent + worktree 隔离** —— 更激进的并行策略
3. 3\. **编辑器原生集成** —— diff 预览、内联编辑体验更好

**通用模式（所有竞品共享）** ：

1. 1\. 项目级配置文件（CLAUDE.md /.cursorrules / copilot-instructions.md）
2. 2\. 工具调用循环（ReAct 模式）
3. 3\. 权限确认机制
4. 4\. MCP 协议支持（逐步趋同）
...
3. 2\. **Dual-Mode 通过 Subagent 而非状态机** ：
...
1. 1\. **Agent 不是产品，Harness 才是。** 模型会被替换，但你精心设计的权限规则、Hook 管道、沙盒配置——这些才是你的护城河。
2. 2\. **约束是力量，不是限制。** 每添加一条精确的权限规则，你就能放心让 Agent 做更多事情。纵深防御的六层让 Claude Code 敢于在生产环境中运行 `rm` 、 `git push` 这样的危险命令——因为每一层都在守护。
3. 3\. **渐进式构建，不要过度设计。** 先用 1 小时搭一个 CLAUDE.md + 基本权限的 Level 1 Harness。遇到痛点了再加 Hook。需要团队协作了再配 MCP 和自定义 Agent。最好的 Harness 是"刚好够用"的 Harness。
